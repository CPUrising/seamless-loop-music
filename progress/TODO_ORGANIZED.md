...
- [ ] **大规模加载优化 (1000+ 首)**:
  - *基准预期*: SQLite 查询 1000-5000 条简单记录耗时通常 < 50ms。
  - *优化方案*: 利用 WPF **UI 虚拟化** (VirtualizingStackPanel) 复用 UI 元素；通过 **异步加载** (Task.Run + Dispatcher) 避免阻塞主线程；仅在列表显示时加载 **必要字段**。
  - *现状*: 策略已明确，待在加载逻辑中完全落实。
...
- [ ] **人声检测**: 排除人声部分的循环点候选。
- [X] **AB 循环支持**: 实现了 "前奏 (Intro) + 循环段 (Loop)" 的物理结构与**重置**逻辑。
...
- [X] **金字塔搜索**: $O(N)$ - 快速由粗到细搜索 (已应用于自研匹配算法中)。
  - **过零点检测**: 已在某些初步尝试中使用。
- [X] **外部工具整合**:
  - **PyMusicLooper**: 已深度整合，通过 `uvx` 实现免安装环境驱动。
  - **批量极致匹配**: 已支持对列表/歌单进行大规模深度分析。
...
- [X] **歌单增强**: 
  - 支持导入多个文件夹，并自动后台预扫描。
- [X] **虚拟歌单系统 (Virtual Playlist System)**:
  - *目标*: 从“物理文件夹映射”切换为“数据库逻辑引用”。
  - *功能*: 已支持歌单逻辑引用、文件多选移动、**歌单/歌曲批量操作**及**拖拽排序**。
...
- [X] **列表刷新**: 已使用 `ObservableCollection` 优化现有列表刷新逻辑。
