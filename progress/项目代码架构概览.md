...
- `CoreEvents.cs`: 定义全局事件（TrackLoadedEvent、PlaybackStateChangedEvent、LoopPointsChangedEvent、PlaylistChangedEvent、TrackMetadataChangedEvent），用于 UI、Services、Core 之间的解耦通信。
...
- `CategoryItem.cs`: 分类项模型，支持 Album / Artist / Playlist 三种分类类型。
