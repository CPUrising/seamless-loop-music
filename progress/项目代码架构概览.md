...
**日期**: 2026-03-28
**状态**: 采用 Repository 仓储模式 + MVVM 完整分离，架构日趋完善。
...
- `CoreEvents.cs`: 定义全局事件，用于 UI、Services、Core 之间的解耦通信。
...
- `Repositories/ITrackRepository.cs`: 曲目仓储接口。
...
- `Views/LibraryView.xaml[.cs]`: 音乐库视图。
...
- `ViewModels/LibraryViewModel.cs`: 音乐库视图的 ViewModel。
- `ViewModels/DetailViewModel.cs`: 曲目详情视图的 ViewModel。

### 📂 Interfaces (接口定义层) - 分散在各层
项目的"契约"，定义服务层对外的抽象接口。（实际位于各服务/仓储目录中）
- `Services/IPlayerService.cs`: 播放器服务的抽象接口。
- `Services/IPlaybackService.cs`: 播放控制服务的抽象接口。
- `Services/IPlaylistManagerService.cs`: 播放列表管理服务的抽象接口。
- `Services/IPlaylistManager.cs`: 播放列表管理器的抽象接口。
- `Services/ILoopAnalysisService.cs`: 循环分析服务的抽象接口。
- `Data/Repositories/ITrackRepository.cs`: 曲目仓储接口。
- `Data/Repositories/IPlaylistRepository.cs`: 播放列表仓储接口。
